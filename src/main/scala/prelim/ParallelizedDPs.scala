...
class ParallelizedDPs[A <: Double, H <: Probability[Double], X](val alpha: A, val dps: IndexedSeq[DirichletProcess[A, H, X]])(val n: Int = dps.map(_.xs.length).sum, val K: Int = dps.length, val a: Double = alpha / dps.length)(_Z: => Double = - Gamma.logGamma(alpha) + K * Gamma.logGamma(a) + Gamma.logGamma(n + alpha)) extends Probability[Double] {

  lazy val Z = _Z

  lazy val evaluate: Double = - Z + dps.map(dp => Gamma.logGamma(dp.xs.length + a) + dp.evaluate).sum
...
object ParallelizedDPs {
  ...
  implicit def _alpha[A <: Double, H <: Probability[Double], X]: Lens[ParallelizedDPs[A, H, X], A] =
    Lens[ParallelizedDPs[A, H, X], A](_.alpha)(alpha => pdp => new ParallelizedDPs[A, H, X](alpha, pdp.dps.map(DirichletProcess._a[A, H, X].set((alpha / pdp.K).asInstanceOf[A])))(pdp.n, pdp.K)())
